...
function loadAdmins() {
    ...
    users.forEach(user => {
        ...
        const deleteDisabled = user.id === currentUser.id ? 'disabled' : '';
        const resetPasswordBtn = currentUser.role === 'super_admin' && user.id !== currentUser.id 
            ? `<button class="btn btn-warning btn-small" onclick="showResetPasswordModal('${user.id}')">パスワードリセット</button>` 
            : '';
        ...
        html += `
            <tr>
                ...
                <td class="actions">
                    ${resetPasswordBtn}
                    <button class="btn btn-danger btn-small" onclick="deleteAdmin('${user.id}')" ${deleteDisabled}>削除</button>
                </td>
            </tr>
        `;
    });
